...
class SimdAddPlugin extends Plugin[VexRiscv]{
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    execute plug new Area {
     ...
     //var bit = Bits(1 bits);
	//bit = B"1";
	var check = Bits(1 bits);
	
//Asm code:
//srl     a5, a5, a3
//andi    a5, a5, 1
// Instruction use:
// rd = dest reg, rs1 = a5, rs2 = a3


var myBool = rs1(rs2.resized)
/*
when(myBool){
check = B"1";
}.otherwise
...
class adipoliPlugin extends Plugin[VexRiscv]{
  //Define the concept of IS_ADI_ADD signals, which specify if the current instruction is destined for ths plugin
  object IS_ADI_ADD extends Stageable(Bool)
  ...
  override def setup(pipeline: VexRiscv): Unit = {
    ...
    //Specify the IS_ADI_ADD default value when instruction are decoded
    decoderService.addDefault(IS_ADI_ADD, False)

    //Specify the instruction decoding which should be applied when the instruction match the 'key' parttern
    decoderService.add(
      //Bit pattern of the new SIMD_ADD instruction
      key = M"0000011----------000-----0110010",
...
rd := (rs1 << 5).resized + rs2
      //When the instruction is a SIMD_ADD one, then write the result into the register file data path.
      when(execute.input(IS_ADI_ADD)) {
        execute.output(REGFILE_WRITE_DATA) := rd.asBits
      }
    }
  }
